  ...
  <div class="repair-manage-page">
      ...
      <el-table v-loading="loading" :data="list" border stripe>
        ...
        <el-table-column label="维修类型" width="90" align="center">
          <template #default="{ row }">
            <el-tag :type="row.repairType === 'MAINTENANCE' ? 'info' : 'primary'">
              {{ row.repairTypeLabel || row.repairType }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="faultDesc" label="故障描述" min-width="180" show-overflow-tooltip />
        <el-table-column prop="repairShopName" label="维修厂" width="150" show-overflow-tooltip>
          <template #default="{ row }">{{ row.repairShopName || '-' }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="140" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="row.orderStatus === 'APPROVED'" link type="primary" v-hasRole="['BIZ_ADMIN','WAREHOUSE']" @click="openStart(row)">
              开始维修
            </el-button>
            <el-button v-if="row.orderStatus === 'REPAIRING'" link type="success" v-hasRole="['BIZ_ADMIN','WAREHOUSE']" @click="openAccept(row)">
              验收
            </el-button>
          </template>
        </el-table-column>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="车辆" prop="vehicleId">
              <el-select v-model="applyForm.vehicleId" placeholder="请选择车辆" filterable style="width: 100%">
          ...
          <el-col :span="12">
            <el-form-item label="维修里程" prop="repairMileage">
              <el-input-number v-model="applyForm.repairMileage" :min="0" :precision="1" :controls="false" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
    ...
    <el-dialog v-model="startVisible" title="开始维修" width="480px" :close-on-click-modal="false">
      <el-form ref="startFormRef" :model="startForm" :rules="startRules" label-width="90px">
        ...
        <el-form-item label="维修厂名称" prop="repairShopName">
          <el-input v-model="startForm.repairShopName" maxlength="100" placeholder="请输入维修厂名称" />
        </el-form-item>
    ...
    <el-dialog v-model="detailVisible" title="维修单详情" width="720px">
      <template v-if="detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="维修时里程(km)">{{ detail.repairMileage ?? '-' }}</el-descriptions-item>
          <el-descriptions-item label="维修厂">{{ detail.repairShopName || '-' }}</el-descriptions-item>
...
import { applyRepair, auditRepair, startRepair, acceptRepair, getRepairPage, getRepairDetail, getVehiclePage } from '@/api/cl'
...
/** 查询 */
const query = reactive({ vehicleId: undefined, repairType: '', orderStatus: '', page: 1, size: 10 })
...
const openApply = () => {
  Object.assign(applyForm, { vehicleId: null, repairType: '', urgencyLevel: '', repairMileage: undefined, faultDesc: '' })
  ...
  applyFormRef.value.validate(async (valid) => {
    ...
    try {
      await applyRepair({
        ...
        repairMileage: applyForm.repairMileage
      })
...
const startForm = reactive({ repairId: null, repairNo: '', repairShopName: '', estimatedCost: undefined })

const startRules = {
  repairShopName: [{ required: true, message: '请输入维修厂名称', trigger: 'blur' }]
}

const openStart = (row) => {
  Object.assign(startForm, { repairId: row.id, repairNo: row.repairNo, repairShopName: '', estimatedCost: undefined })
  startFormRef.value && startFormRef.value.clearValidate()
  ...
  startFormRef.value.validate(async (valid) => {
    ...
    try {
      await startRepair({
        ...
        repairShopName: startForm.repairShopName,
